...
El proyecto tiene una finalidad docente y sirve para practicar PHP 8 con programación orientada a objetos, arquitectura MVC, sesiones, JavaScript con `fetch` e integración de APIs mediante cURL.
...
- El historial de la conversación se guarda en la sesión de PHP.
- El modelo se cambia modificando una sola variable de configuracion.
...
El modelo debe recibir los mensajes anteriores de la conversación para mantener el contexto. El historial debe almacenarse en `$_SESSION`.
...
El identificador del modelo debe leerse desde `OPENROUTER_MODEL` en el archivo `.env`. Debe ser posible cambiar de modelo modificando únicamente esa variable.

### RF13. Seguridad de la clave

La clave de OpenRouter debe leerse desde `OPENROUTER_API_KEY` en el archivo `.env`. La clave nunca debe:
...
Todo contenido recibido desde el usuario o desde el modelo debe mostrarse usando `textContent` o un mecanismo equivalente de escapado. No se permite insertar contenido del modelo directamente mediante `innerHTML`.
...
La aplicación debe mostrar mensajes comprensibles en estos casos:
- Falta la API key.
- API key incorrecta.
- Error de conexión.
...
El historial debe guardarse en `$_SESSION`.

La clase `Conversacion` será la única responsable de gestionar el historial. Debe proporcionar estos métodos:
...
```
y abrir `http://localhost:8000`, debe mostrarse la pantalla del chat.
...
Al modificar únicamente `OPENROUTER_MODEL` en `.env`, la aplicación debe utilizar el nuevo modelo.
